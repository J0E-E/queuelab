...
permanently stops the writer. It stops only when the task is cancelled. The shared subscribe loop
(subscribe, re-subscribe-after-a-pause, close) lives in
...
async def run_durable_writer(queue: JobQueue, database: Database) -> None:
    ...
    and its re-subscribe-after-a-pause guard live in :func:`run_state_subscriber`; persisting a
    single event is itself best-effort (see :func:`_handle_message`).
    """
    await run_state_subscriber(
        queue, STATE_CHANNEL, partial(_handle_message, database), name="durable-writer"
    )
