  ...
  backend/                FastAPI app (api + autoscaler entrypoints)
    app/
      ...
      queue/              the custom Redis queue (shared protocol)
        ...
        scripts/          all-at-once or not-at-all (atomic) Lua scripts (claim, reap, ack)
...
- `ql:queue:ready` — a **List** of job IDs (first-in-first-out (FIFO)). Producers add to the
  left (`LPUSH`); workers claim.
- `ql:queue:delayed` — a **Sorted Set** scored by "ready-at" epoch ms, for retry
  backoff. A recovery sweep (reaper) moves due jobs back to `ready`.
...
- `ql:leases` — a **Sorted Set** of in-flight job IDs scored by their claim deadline
  (lease, in epoch ms): the time limit before a claim expires (the **visibility timeout**).
  The recovery sweep (reaper) finds entries past their deadline to detect a dead worker's
  job and put it back on the queue (requeue). (A TTL key can't be enumerated once it
  expires, so claims (leases) are tracked here explicitly rather than as per-job TTL keys.)
- `ql:counts` — a Hash of live state counts (queued/running/completed/failed/retrying)
  for O(1) dashboard reads.

**Claiming** uses `BLMOVE ql:queue:ready ql:processing:{worker_id} RIGHT LEFT` (a blocking
pop plus an all-at-once or not-at-all (atomic) move into the worker's processing list), then
a Lua script sets the job to `running`, stamps `started_at`, and adds the job to `ql:leases`
scored `now + visibility_timeout`. This is the real, explainable claiming mechanic the lab
exists to show — it happens all-at-once or not at all (atomic), the job runs one or more
times (at-least-once), and each claim has a time limit before it expires (a visibility
timeout).
...
**Recovery sweep (reaper) / claim (lease) recovery (how chaos recovers):** a background loop
in the api process (a) moves due jobs from `delayed` to `ready`; (b) puts back on the queue
(re-queues) jobs whose `ql:leases` deadline has passed — when a worker is destroyed mid-job,
its claim deadline (lease) lapses, the job is treated as a failed attempt and put back on the
queue (respecting `max_retries`). This is what makes "destroy a worker → job is retried" real
...
- Graceful shutdown (SIGTERM) fails the in-flight job and puts it back on the queue
  (nack/requeue); **hard destroy** (SIGKILL via Docker) does not — that path exercises the
  recovery sweep that picks up jobs whose claim expired (the lease-expiry reaper), which is
...
real mechanics (custom Redis queue, claiming plus the time limit before a claim expires
(visibility timeout), retries/recovery,
...
and `counts` → workers grab and move a job into their processing list to claim it
(`BLMOVE`) → run sim → ack → `counts`/`job` deltas + durable
...
| 4 | Queue mechanics | **Custom queue on Redis primitives** | Arq; RQ / Dramatiq | Maximum teaching value — claiming, the time limit before a claim expires (visibility timeout), retries, recovery are real and explainable in-context, which is the point of the lab. |
...
- **At-least-once delivery** (every job runs one or more times). The model built on a
  time-limited claim that expires (the lease / visibility timeout) can run a job twice
...
4. Destroy a worker mid-job → confirm the in-flight job is retried (its claim, or lease,
   expired), a
...
7. **Reaper** (the recovery sweep) — moving delayed jobs to ready (promotion) and recovering
   jobs whose claim expired (lease-expiry recovery), the chaos-recovery path.
